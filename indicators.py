# indicators.py - Institutional Logic
import pandas as pd
import numpy as np
...
def calc_demark(df):
    """
    DeMark Sequential: 
    - Setup 9: 9 consecutive closes >/< close 4 bars ago
    - Perfected: Bar 8 or 9 high/low exceeds bar 6 & 7
    """
    try:
        df = df.copy()
        c = df['Close'].values
...
def calc_adx(df, period=14):
    try:
        ...
        dx = 100 * abs(p_di - m_di) / (p_di + m_di).replace(0, 1)
