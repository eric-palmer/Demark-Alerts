...
def analyze_ticker(ticker, is_portfolio=False):
    try:
        ...
        return {
            ...
            'demark': dm_sig, 'rsi': rsi_sig, 'squeeze': sq_sig, 'fib': fib, 'tech': tech_alert,
            'trend': trend, 'verdict': verdict, 'target': p_target, 'stop': p_stop,
            'rsi_val': last_d['RSI'], 'macd': macd_val,
...
if __name__ == "__main__":
    ...
    if macro:
        ...
        phase = "NEUTRAL"
        if roc_med > 0: phase = "REBOUND (Early Cycle)" if accel > 0 else "SPECULATION (Late Cycle)"
        else: phase = "TURBULENCE (Contraction)" if accel < 0 else "ACCUMULATE CREDIT (Bottoming)"
